...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node *copy_nodes_impl(Node *node) {
      if (!node) {
        ...
        Node* new_friend = new Node(T(), nullptr, nullptr);
        new_friend->datum = node->datum;
        if (node->right) {
            new_friend->right = copy_nodes_impl(node->right);
        }
        if (node->left) {
            new_friend->left = copy_nodes_impl(node->left);
        }
        return new_friend;
  }
  ...
  static void destroy_nodes_impl(Node *node) {
      if (node) {
        Node* victim = new Node(T(), nullptr, nullptr);
          if (node->right) {
        ...
        delete node;
        if (victim->left) {
            destroy_nodes_impl(victim->left);
        }
        if (victim->right) {
            destroy_nodes_impl(victim->right);
        }
        delete victim;
      }
  }
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
      if (!node) {
          ...
          else return insert_impl(node->left, item, less);
      }
      else  {
          if (!node->right) {
              Node* insert = new Node(item, nullptr, nullptr);
          ...
          else insert_impl(node->right, item, less);
      }
      return node;
  }
