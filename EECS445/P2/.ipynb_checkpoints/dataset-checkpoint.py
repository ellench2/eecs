...
class ImageStandardizer(object):
    ...
    def fit(self, X):
        ...
        # for each channel:
            # compute mean, iterating over height and width, subtract that away from each element
            # compute stdev, same, divide each element by it
        # redsum = 0
        # bluesum = 0
        # greensum = 0
        # for i in range(X.shape[0]):
        ...
        #             greensum += X[i,k,l,1]
        #             bluesum += X[i,k,l,2]
        # redavg = redsum / (X.shape[0] * X.shape[1] * X.shape[2])
        # greenavg = greensum / (X.shape[0] * X.shape[1] * X.shape[2])
        # blueavg = bluesum / (X.shape[0] * X.shape[1] * X.shape[2])
        # reddiff = 0
        # greendiff = 0
        # bluediff = 0
        # for i in range(X.shape[0]):
        #     for k in range(X.shape[1]):
        #         for l in range(X.shape[2]):
        #             reddiff += (X[i,k,l,0] - redavg)**2
        #             greendiff += (X[i,k,l,1] - greenavg)**2
        #             bluediff += (X[i,k,l,2] - blueavg)**2
        # redstd = np.sqrt(reddiff / (X.shape[0] * X.shape[1] * X.shape[2] - 1))
        # greenstd = np.sqrt(greendiff / (X.shape[0] * X.shape[1] * X.shape[2] - 1))
        # bluestd = np.sqrt(bluediff / (X.shape[0] * X.shape[1] * X.shape[2] - 1))
