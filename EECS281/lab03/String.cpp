...
String& String::erase(size_t pos, size_t len) {
    ...
    unsigned long end = std::min(pos + len, sz);
    for (size_t i = 0; i < sz-end; i++) {
...
String& String::insert(size_t pos, const String& str) {
    ...
    if (this == &str) {
        ...
        String temp = *this;
        this->erase(pos, sz+1)+=str1;
        *this += temp.erase(0,pos);
        return *this;
    }
    if (pos == 0) {
        if (sz == 0) {
...
//    }
//    size_t min = npos;
//        for (unsigned long j = 0; j < str.sz; j++) {
//            for (size_t i = 0; i < str.sz; i++) {
//                unsigned long temp_min = find(&str[i], pos);
//                if (temp_min < sz && temp_min < min) {
//                    min = temp_min;
//                }
//            }
//            if (min < npos) {
...
//    }
//
//    else return static_cast<size_t>(min);
...
//    }
//
//    else return static_cast<size_t>(min); // GET RID OF THIS LINE WHEN YOU BEGIN IMPLEMENTING FIND_LAST_OF (this is for compile purposes only)
    size_t temp = 0;
    size_t end = std::min(sz,pos);
