...
public:
    ...
    public:
        explicit table_entry_comp(sillytable input) {
            ...
            cin >> compare;
            if (input.columns[col_to_check].first == "double") {
                double temp_in;
                cin >> temp_in;
                TableEntry val_in{temp_in};
                val.push_back(val_in);
            }
            if (input.columns[col_to_check].first == "int") {
                int temp_in;
                cin >> temp_in;
                TableEntry val_in{temp_in};
                val.push_back(val_in);
            }
            if (input.columns[col_to_check].first == "bool") {
                string temp_in;
                cin >> temp_in;
                bool real_temp_in;
                if (temp_in == "true") {
                ...
                TableEntry val_in{real_temp_in};
                val.push_back(val_in);
            }
            if (input.columns[col_to_check].first == "string") {
                string temp_in;
                cin >> temp_in;
                TableEntry val_in{temp_in};
                val.push_back(val_in);
    ...
    void print_rows(int num_cols) {
        if (quiet) {
            ...
            for (int i = 0; i < num_cols; i++) {
                ...
                cin >> cols_print_in;
                if (find_if(columns.begin(), columns.end(), [cols_print_in] (pair<string, string> a) {return a.second == cols_print_in;}) == columns.end()) {
                    std::cout << "Error during PRINT: " << cols_print_in << " does not name a column in " << name << "\n";
                    string trash;
                    getline(cin, trash);
                    return;
                }
                else { cols_print.push_back(static_cast<uint>(find_if(columns.begin(), columns.end(), [cols_print_in] (pair<string, string> a) {return a.second == cols_print_in;}) - columns.begin()));
                }
            }
            string trash;
            cin >> trash;
...
//            return comp(count - 1, *this);}));
    ...
    void generate_index(string index) {
        ...
        uint index_col = static_cast<uint>(find_if(columns.begin(), columns.end(), [col_in] (pair<string, string> a) {return a.second == col_in;}) -
                    columns.begin());
        ...
        if (index[0] == 'h') {
            which_index = true;
            for (uint i = 0; i < static_cast<uint>(data.size()); i++) {
                if (hash_index.find(data[i][index_col]) != hash_index.end()) {
                    hash_index.find(data[i][index_col])->second.push_back(i);
                }
                else {
                    vector<uint> insert_vec;
                    insert_vec.push_back(i);
